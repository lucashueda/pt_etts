...
if __name__ == '__main__':
    ...
    for file in os.listdir(cpqd_path):
        ...
        if(file not in ['neutro_ruim', 'old']):
    ...
    df = pd.DataFrame({'wav_path':wav_dirs, 'text': texts, 'emb_id': emb_ids})


    df_train, df_val, speaker_id, speaker_id = train_test_split(df, df['emb_id'], test_size = 0.01, random_state = 42)

    out_train = os.path.join(args.output_directory, 'alfredo_train.csv')
    out_val = os.path.join(args.output_directory, 'alfredo_val.csv')

    df_train.to_csv(out_train, index = False  , sep="|")
    df_val.to_csv(out_val, index = False, sep='|')
