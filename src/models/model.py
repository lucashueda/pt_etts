...
from src.utils.utils import to_gpu, get_mask_from_lengths
...
### 
### Decoder with post-encoder speaker embedding (just embedding layer)
###
...
# Criando camada de embedding de speaker

# class SpeakerEmbedding(nn.Module):
#     def __init__(self, hparams):
#         self.emb_max_size = hparams.emb_max_size
...
class Tacotron2(nn.Module):
    def __init__(self, hparams):
        ...
        self.decoder = Original_Decoder(hparams)
...
class Tacotron2SSE(nn.Module):
    ...
    def parse_batch(self, batch):
        ...
        gate_padded = to_gpu(gate_padded).float()
        output_lengths = to_gpu(output_lengths).long()
        embedds = to_gpu(embeds).long()
        return (
            (text_padded, input_lengths, mel_padded, max_len, output_lengths, embedds),
            (mel_padded, gate_padded))
...
##
##  Tacotron with speaker embedding just after the encoder layer 
##
...
class Tacotron2_EncSpeakEmb(nn.Module):
    ...
    def inference(self, inputs, embedds):
        ...
        print(embedded_inputs.shape, encoder_outputs.shape, speaker_embedd_input.shape, speaker_embedd_input.repeat((1,encoder_outputs.size(1),1)).shape)
